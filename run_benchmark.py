...
def main():
    ...
    for size in SIZES:
        ...
        for _ in range(REPEATS):
            ...
            run_command(f"python3 {PYTHON_SCRIPT} --size {size} --steps {STEPS} --append-to {OUTPUT_FILE}")
...
def plot_results():
    ...
    with open(OUTPUT_FILE, "r") as f:
        ...
        for row in reader:
            lang = row["Language"]
            size = int(row["Size"])
            time_sec = float(row["TimeSeconds"])
    ...
    plt.plot(sizes, rust_times, marker='o', label='Rust (Rayon Parallel)')
    plt.plot(sizes, python_times, marker='s', label='Python (Numpy Vectorized)')
    plt.plot(sizes, go_times, marker='^', label='Go (Goroutines Parallel)')
    
    plt.xlabel('Grid Size (NxN)')
    plt.ylabel('Execution Time (seconds) 100 steps')
    plt.title('Performance Benchmark: Rust vs Go vs Python')
    plt.legend()
    plt.grid(True)
